...
glib_wrapper! {
    pub struct Component(Object<ibus_sys::IBusComponent, ibus_sys::IBusComponentClass, ComponentClass>) @extends Serializable, Object, gobject::InitiallyUnowned;
...
impl Component {
    ...
    pub fn from_xml_node(node: &mut XML) -> Component {
        assert_initialized_main_thread!();
        unsafe {
            from_glib_none(ibus_sys::ibus_component_new_from_xml_node(node.to_glib_none_mut().0))
        }
    }
...
impl<O: IsA<Component>> ComponentExt for O {
    ...
    fn get_observed_paths(&self) -> Vec<ObservedPath> {
        unsafe {
            FromGlibPtrContainer::from_glib_container(ibus_sys::ibus_component_get_observed_paths(self.as_ref().to_glib_none().0))
        }
    }
